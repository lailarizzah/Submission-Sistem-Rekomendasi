...
Tahap ini dimulai dengan membuat preprocessor menggunakan ColumnTransformer yang memungkinkan kita menerapkan transformasi yang berbeda pada kolom yang berbeda. Cara ini lebih efisien daripada melakukan transformasi satu per satu. Berikut beberapa proses yang dilakukan pada data preparation:
1. Menggabungkan (Combine) Fitur yang Relevan: Menciptakan representasi numerik tunggal dari setiap produk yang mencakup semua atribut.
2. Encoding Fitur Kategorikal (dan Teks Bebas): Menerapkan One-Hot Encoding dan TF-IDF pada kolom 'Label', 'Brand', 'Name', 'Ingredients'.
3. Standardisasi/Scaling Fitur Numerik: Menerapkan StandardScaler pada 'Rank' dan 'Price'.
4. Membuat Representasi Vektor Produk: Menggabungkan hasil dari langkah 2 dan 3 (serta fitur biner yang sudah ada) menjadi satu vektor per produk.
"""
...
# Kolom yang dibutuhkan: ID Pengguna, ID Item, Rating
# Di data review_produk: 'Reviewer', 'Product', 'Rating'
required_columns = ['Review', 'Product', 'Rating']
if not all(col in review_data_for_cf.columns for col in required_columns):
    print("Kolom yang dibutuhkan untuk Collaborative Filtering ('Review', 'Product', 'Rating') tidak lengkap.")
    # Tambahkan penanganan error atau keluar dari sel jika kolom tidak ada
else:
    ...
    print(f"Jumlah reviewer unik: {review_data_for_cf['Review'].nunique()}")
    print(f"Jumlah produk unik: {review_data_for_cf['Product'].nunique()}")


    # 2: Transformasi Data untuk Surprise
    # surprise.Reader membaca dataset dari pandas DataFrame
    # Definisi skala rating (misalnya, dari 1 hingga 5)
    min_rating = review_data_for_cf['Rating'].min()
    max_rating = review_data_for_cf['Rating'].max()
    reader = Reader(rating_scale=(min_rating, max_rating))

    # Muat data dari DataFrame ke objek Dataset Surprise
    # Format: user_id, item_id, rating
    data = Dataset.load_from_df(review_data_for_cf[['Review', 'Product', 'Rating']], reader)
...
def recommend_products(product_name, top_n=5):
    if product_name not in kosmetik_info_cleaned['Product'].values:
        print(f"Produk '{product_name}' tidak ditemukan.")
        return

    # Index produk
    idx = kosmetik_info_cleaned[kosmetik_info_cleaned['Product'] == product_name].index[0]

    # Hitung skor similarity
    sim_scores = cosine_similarity(kosmetik_processed[idx], kosmetik_processed).flatten()

    # Ambil indeks top_n + 1 karena produk itu sendiri akan jadi skor 1.0
    sim_indices = sim_scores.argsort()[-top_n-1:][::-1]

    # Buang produk itu sendiri
    sim_indices = [i for i in sim_indices if i != idx][:top_n]

    # Ambil hasil
    hasil = kosmetik_info_cleaned.iloc[sim_indices][['Product', 'Category', 'Merk']]
    hasil['Similarity Score'] = sim_scores[sim_indices]
...
for product in sample_products:
    ...
    rekomendasi = recommend_products(product, top_n=5)
...
# --- Langkah 3: Model Collaborative Filtering (SVD) ---
# Gunakan model SVD (Singular Value Decomposition)
algo = SVD()

print("\nMelakukan cross-validation untuk model SVD...")
# Evaluasi model menggunakan Cross-Validation (opsional, tapi direkomendasikan)
...
# Latih model pada seluruh dataset untuk digunakan merekomendasikan
trainset = data.build_full_trainset()
algo.fit(trainset)

print("\nModel SVD selesai dilatih pada seluruh dataset.")

# --- Langkah 4: Sistem Rekomendasi ---
...
def get_collaborative_recommendations(user_id, n_recommendations=10, watched_items=None):
    ...
    Args:
        user_id (str): ID pengguna yang ingin direkomendasikan.
        n_recommendations (int): Jumlah rekomendasi yang diinginkan.
        watched_items (list, optional): Daftar item yang sudah ditonton/dinilai oleh pengguna.
                                        Jika None, akan dicari dari data trainset.

    Returns:
        list: Daftar pasangan (Nama Produk, Prediksi Rating) untuk n produk teratas
              yang belum dinilai oleh pengguna.
    """
    # Jika watched_items tidak disediakan, cari dari data asli
    if watched_items is None:
        user_ratings = review_data_for_cf[review_data_for_cf['Review'] == user_id]
        watched_items = user_ratings['Product'].tolist()

    # Dapatkan daftar semua item unik
    all_items = review_data_for_cf['Product'].unique()

    # Filter item yang belum dinilai oleh pengguna
    items_to_predict = [item for item in all_items if item not in watched_items]

    predictions = []
    # Prediksi rating untuk item yang belum dinilai
...
# --- Contoh Penggunaan ---
# Temukan contoh user_id dari data
if not review_data_for_cf.empty:
    example_user_id = review_data_for_cf['Review'].iloc[0]  # Ambil user ID dari baris pertama sebagai contoh
    print(f"\nMemberikan rekomendasi Collaborative Filtering untuk pengguna: '{example_user_id}'")

    # Dapatkan daftar item yang sudah dinilai oleh pengguna contoh
    user_watched_items = review_data_for_cf[review_data_for_cf['Review'] == example_user_id]['Product'].tolist()
    print(f"Produk yang sudah dinilai pengguna '{example_user_id}': {user_watched_items[:5]}...")  # Tampilkan beberapa
